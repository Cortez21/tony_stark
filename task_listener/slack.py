...
def send_message(message):
    write_log('Sending message to slack channel...', is_new_line=False)
    url = f'https://slack.com/api/chat.postMessage?token={get_token()}&channel={get_channel()}&text={message}&pretty=1'
    response = requests.post(url)
    write_log('Success' if response.status_code == 200 and response.json()['ok'] else f'Fail ({response.json()["error"]})')
    return response


def get_permalink(message_ts):
    write_log(f'Getting permalink for {message_ts}...', is_new_line=False)
    ...
    write_log('Success' if response.status_code == 200 and response.json()['ok'] else 'Fail')
    return response.json()['permalink']


def add_reaction(message_ts, reaction):
    write_log(f'Adding {reaction} reaction to {message_ts} message...', is_new_line=False)
    url = f'https://slack.com/api/reactions.add?token={get_token()}&channel={get_channel()}&name={reaction}&timestamp={message_ts}&pretty=1'
    response = requests.post(url)
    write_log('Success' if response.json()['ok'] else 'Fail')
    return response.json()['ok']


def get_reactions(message_ts):
    write_log(f'Getting reactions for {message_ts}...', is_new_line=False)
    url = f'https://slack.com/api/reactions.get?token={get_token()}&channel={get_channel()}&timestamp={message_ts}&pretty=1'
    response = requests.post(url)
    try:
        write_log('Success' if response.json()['message']['reactions'] else 'Fail')
...
def remove_reaction(message_ts, reaction):
    write_log(f'Remove {reaction} reaction for {message_ts}...', is_new_line=False)
